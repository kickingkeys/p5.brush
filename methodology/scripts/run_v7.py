#!/usr/bin/env python3
"""
Phase 4e — v7 Recipes validation run.

Same 12 prompts × 5 models × 1 condition (v7 = cookbook-recipes HEAD after v7 edits).
v7 targets two v5 pathologies: (1) over-prescriptive subject-example lists reading as
category boundaries — reframed "When NOT" as "Two ways density can emerge" principle,
softened Form/Field trigger vocab to "e.g."; (2) Form watercolor p01 fill-count collapse
v3→v5 — added explicit "Form watercolor always uses ≥3 passes" bullet.
Writes to renders/v7/ and manifest_v7.json. Earlier artifacts untouched.

Usage:
  python3 scripts/run_v7.py           # pilot: claude_sonnet_46 only
  python3 scripts/run_v7.py --all     # all 5 models
  python3 scripts/run_v7.py --models claude_sonnet_46,gpt_54
"""
import argparse
import json
import os
...
MANIFEST_PATH = ROOT / "manifest_v7.json"
RENDERS = ROOT / "renders" / "v7"
...
def load_v7_condition() -> str:
    # v7 = cookbook-recipes HEAD (post-v7 edits)
...
def run(selected_models: dict) -> None:
    ...
    print("Loading v7 condition from git ref cookbook-recipes...")
    system = load_v7_condition()
    print(f"  v7 system prompt: {len(system)} chars")
    ...
    for model_name, cfg in selected_models.items():
        for prompt in PROMPTS["prompts"]:
            key = f"{model_name}__v7__{prompt['id']}"
    ...
    print(f"\n{'='*60}\nV7 RUN COMPLETE — {datetime.now().isoformat()}\n{'='*60}")
    ...
    for m in selected_models:
        g = sum(1 for k, v in manifest.items() if k.startswith(f"{m}__v7__") and v.get("generated"))
        r = sum(1 for k, v in manifest.items() if k.startswith(f"{m}__v7__") and v.get("rendered"))
        print(f"  {m:20s} v7: {g}/12 gen, {r}/12 render")
