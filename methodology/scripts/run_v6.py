#!/usr/bin/env python3
"""
Phase 4e — v6 Recipes validation run.

Same 12 prompts × 5 models × 1 condition (v6 = cookbook-recipes HEAD after v6 edits).
v6 targets two v5 pathologies: (1) over-prescriptive subject-example lists reading as
...
Writes to renders/v6/ and manifest_v6.json. Earlier artifacts untouched.

Usage:
  python3 scripts/run_v6.py           # pilot: claude_sonnet_46 only
  python3 scripts/run_v6.py --all     # all 5 models
  python3 scripts/run_v6.py --models claude_sonnet_46,gpt_54
"""
import argparse
import json
import os
...
MANIFEST_PATH = ROOT / "manifest_v6.json"
RENDERS = ROOT / "renders" / "v6"
...
def load_v6_condition() -> str:
    # v6 = cookbook-recipes HEAD (post-v6 edits)
...
def run(selected_models: dict) -> None:
    ...
    print("Loading v6 condition from git ref cookbook-recipes...")
    system = load_v6_condition()
    print(f"  v6 system prompt: {len(system)} chars")
    ...
    for model_name, cfg in selected_models.items():
        for prompt in PROMPTS["prompts"]:
            key = f"{model_name}__v6__{prompt['id']}"
    ...
    print(f"\n{'='*60}\nV6 RUN COMPLETE — {datetime.now().isoformat()}\n{'='*60}")
    ...
    for m in selected_models:
        g = sum(1 for k, v in manifest.items() if k.startswith(f"{m}__v6__") and v.get("generated"))
        r = sum(1 for k, v in manifest.items() if k.startswith(f"{m}__v6__") and v.get("rendered"))
        print(f"  {m:20s} v6: {g}/12 gen, {r}/12 render")
