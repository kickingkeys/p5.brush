#!/usr/bin/env python3
"""
Phase 4d — v5 Recipes validation run.

Same 12 prompts × 5 models × 1 condition (v5 = cookbook-recipes HEAD after v5 edits).
v5 targets v3 regressions (p04 cpencil, p05 pen forest, p07 spray wall, p12 swarm):
generalized "When NOT to apply graduated density" across mediums, added Spray Chaos
mode, capped cpencil at 3 colors, capped pen per-zone at 1-2 layers, narrowed
Particle Fields anchor rule. Writes to renders/v5/ and manifest_v5.json.
v1/v2/v3 artifacts remain untouched.

Usage:
  python3 scripts/run_v5.py           # pilot: claude_sonnet_46 only
  python3 scripts/run_v5.py --all     # all 5 models
  python3 scripts/run_v5.py --models claude_sonnet_46,gpt_54
"""
import argparse
import json
import os
...
def load_v5_condition() -> str:
    # v5 = cookbook-recipes HEAD (post-v5 edits)
...
def run(selected_models: dict) -> None:
    ...
    print("Loading v5 condition from git ref cookbook-recipes...")
    system = load_v5_condition()
    print(f"  v5 system prompt: {len(system)} chars")
    ...
    for model_name, cfg in selected_models.items():
        for prompt in PROMPTS["prompts"]:
            key = f"{model_name}__v5__{prompt['id']}"
    ...
    print(f"\n{'='*60}\nV5 RUN COMPLETE — {datetime.now().isoformat()}\n{'='*60}")
    ...
    for m in selected_models:
        g = sum(1 for k, v in manifest.items() if k.startswith(f"{m}__v5__") and v.get("generated"))
        r = sum(1 for k, v in manifest.items() if k.startswith(f"{m}__v5__") and v.get("rendered"))
        print(f"  {m:20s} v5: {g}/12 gen, {r}/12 render")
