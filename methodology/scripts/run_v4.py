#!/usr/bin/env python3
"""
Phase 4d — v4 Recipes validation run.

Same 12 prompts × 5 models × 1 condition (v4 = cookbook-recipes HEAD after v4 edits).
v4 targets v3 regressions (p04 cpencil, p05 pen forest, p07 spray wall, p12 swarm):
...
Particle Fields anchor rule. Writes to renders/v4/ and manifest_v4.json.
v1/v2/v3 artifacts remain untouched.

Usage:
  python3 scripts/run_v4.py           # pilot: claude_sonnet_46 only
  python3 scripts/run_v4.py --all     # all 5 models
  python3 scripts/run_v4.py --models claude_sonnet_46,gpt_54
"""
import argparse
import json
import os
...
MANIFEST_PATH = ROOT / "manifest_v4.json"
RENDERS = ROOT / "renders" / "v4"
...
def load_v4_condition() -> str:
    # v4 = cookbook-recipes HEAD (post-v4 edits)
...
def run(selected_models: dict) -> None:
    ...
    print("Loading v4 condition from git ref cookbook-recipes...")
    system = load_v4_condition()
    print(f"  v4 system prompt: {len(system)} chars")
    ...
    for model_name, cfg in selected_models.items():
        for prompt in PROMPTS["prompts"]:
            key = f"{model_name}__v4__{prompt['id']}"
    ...
    for m in selected_models:
        g = sum(1 for k, v in manifest.items() if k.startswith(f"{m}__v4__") and v.get("generated"))
        r = sum(1 for k, v in manifest.items() if k.startswith(f"{m}__v4__") and v.get("rendered"))
        print(f"  {m:20s} v4: {g}/12 gen, {r}/12 render")
