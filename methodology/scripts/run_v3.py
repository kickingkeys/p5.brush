#!/usr/bin/env python3
"""
Phase 4d — v3 Recipes validation run.

Same 12 prompts × 5 models × 1 condition (v3 = cookbook-recipes HEAD after v3 edits).
v3 split the Watercolor recipe into Form/Field sub-recipes and added a
"When NOT to apply graduated density" bullet. Writes to renders/v3/ and
manifest_v3.json. v1 and v2 artifacts remain untouched.

Usage:
  python3 scripts/run_v3.py           # pilot: claude_sonnet_46 only
  python3 scripts/run_v3.py --all     # all 5 models
  python3 scripts/run_v3.py --models claude_sonnet_46,gpt_54
"""
import argparse
import json
import os
...
def load_v3_condition() -> str:
    # v3 = cookbook-recipes HEAD (post-v3 edits)
...
def run(selected_models: dict) -> None:
    ...
    print("Loading v3 condition from git ref cookbook-recipes...")
    system = load_v3_condition()
    print(f"  v3 system prompt: {len(system)} chars")
    ...
    for model_name, cfg in selected_models.items():
        for prompt in PROMPTS["prompts"]:
            key = f"{model_name}__v3__{prompt['id']}"
    ...
    for m in selected_models:
        g = sum(1 for k, v in manifest.items() if k.startswith(f"{m}__v3__") and v.get("generated"))
        r = sum(1 for k, v in manifest.items() if k.startswith(f"{m}__v3__") and v.get("rendered"))
        print(f"  {m:20s} v3: {g}/12 gen, {r}/12 render")
